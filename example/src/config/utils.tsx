...
/* Example of function that returns all the menu items, takes item as prop */
export const customMenus = (item: Content) => [
  { icon: 'heart', value: '1', onPress: () => console.log(item) } as UIMenuItem,
  { icon: 'heart', value: '2', onPress: () => console.log(item) } as UIMenuItem,
  { icon: 'heart', value: '3', onPress: () => console.log(item) } as UIMenuItem,
];
...
export const getVideos = async (index: number) => {
  try {
    await delay(1500);
    const newContent = mediaJSON.categories[0]?.videos.slice(
      index * videosPerPage,
      index * videosPerPage + 2
    );
    return newContent;
  } catch (error) {
...
/* Example of data returned from an API */
